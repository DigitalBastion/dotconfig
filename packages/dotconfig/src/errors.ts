...
export class ParseError
	...
	implements StandardSchemaV1.Issue
{
	constructor(
		message: string,
...
export class ParseErrors extends ConfigurationAggregateError {
	constructor(
		...
		data: ParseErrorsData,
	) {
		super(
			issues.map((issue) => new ParseError(issue.message, issue.path)),
			`Configuration parse errors at "${data.configurationPath}" path.`,
			{ cause: data.cause },
		);
	}
}
