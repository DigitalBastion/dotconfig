...
describe("GetChildKeys", () => {
	test("Can chain empty keys", async () => {
		...
		const chainedConfigurationSource = new ChainedConfigurationSource(
			configurationRoot,
		);

		const chainedConfiguration = new ChainedConfigurationProvider(
			chainedConfigurationSource,
		);
...
test.each([true, false])(
	...
	async (removePath) => {
		...
		const dict = Object.fromEntries(
			iterateConfigurationEntries(config, removePath),
		);
...
test("New configuration root may be built from existing with duplicate keys", async () => {
	...
	const newConfigurationRoot = await new ConfigurationBuilder()
		.addMemoryCollection(new Map(configurationRoot))
		.build();
...
class TestMemorySourceProvider
	extends MemoryConfigurationProvider
...
test("Can get configuration children", async () => {
	...
	expect(configSections.find((x) => x.key === "DB1")?.get("Connection1")).toBe(
		"MemVal1",
	);
	expect(configSections.find((x) => x.key === "DB1")?.get("Connection2")).toBe(
		"MemVal2",
	);
	expect(configSections.find((x) => x.key === "DB2Connection")?.value).toBe(
		"MemVal3",
	);
...
test.each(["Value1", ""])(
	"Key with value and without children exists as section",
	async (value) => {
		// Arrange
...
class NullReloadTokenConfigSource
	implements IConfigurationSource, IConfigurationProvider
{
...
test("Iterate configuration entries", async () => {
	// Arrange
	const dict = new Map<string, string>([["Mem1:KeyInMem1", "ValueInMem1"]]);
